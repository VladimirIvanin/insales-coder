...
На 25.09.2026 handle `tvist-v1` подключал ядро и модули версии **1.16.11**. Имя записи `tvist@1.0.0` и документация более новой версии не определяют API текущей сборки. Перед использованием нового или редко встречающегося метода получи актуальные `js_file_url` через `read_theme_catalog(kind=libraries)` и сверься с подключаемым кодом.
